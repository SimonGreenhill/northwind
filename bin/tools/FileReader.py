...
class FileReader(object):
    ...
    boundary_tokens = set([Token(" ", phoneme_type="punctuation")])
    ...
    default_tokens = set(
        [
            ...
            Token(
                unicodedata.lookup("DOUBLE VERTICAL LINE"), phoneme_type="punctuation"
            ),
        ]
    )
    ...
    def __init__(self, filename=None, preferred_transcript="broad"):
        ...
        if self.filename:
            self.read(filename)
            self.language = os.path.splitext(os.path.basename(filename))[0]
    ...
    def __cmp__(self):  # pragma: no cover
        return self.filename

    def __lt__(self, other):
        return self.filename < other.filename

    # this simple function makes some of the code below more legible
    def join(self, x):
        return " ".join([_.strip() for _ in x]) if x else ""
    ...
    def _parse_section(self, section):
        ...
        if section not in self.data:  # pragma: no cover
            self.data[section] = []
        return section
    ...
    def _process_standardisation(self):
        ...
        for section in self._to_standardise:
            if self.data[section] is not None:
                ...
                self.data[section] = [self.standardise(l) for l in self.data[section]]
    ...
    def errors(self):
        """
        Returns a list of errors (i.e. things missing from the inventory
        but seen in tokens
        """
        if self.data["tokenised_transcript"] is None:  # pragma: no cover
            return []
        else:
    ...
    def read(self, filename):
        ...
        self.other_symbols = self.parse_list(
            self.data["Other Symbols"], toktype="other"
        )
    ...
    def parse_transcript(self, transcript):
        ...
        while len(transcript):
            ...
            self.logger.debug(
                'Transcript: BEGIN Round %d with "%s"' % (loopcount, char)
            )
            ...
            if not self.is_boundary(char):
                while len(transcript) and self.is_combining(transcript[0]):
                    ...
                    self.logger.debug(
                        'Transcript: extended with combining to "%s"' % char
                    )
            ...
            # 4. exact = 0, no possible future matches
                ...
                if len(previous) == 1:  # had one previous match
                    self.logger.debug(
                        "Transcript: -> #5a (append previous): %r" % previous
                    )
                ...
                # otherwise then append a missing token.
                    ...
                    if maximal:
                        self.logger.debug(
                            "Transcript: -> #5b1 (append maximal submatch)"
                        )
                        ...
                        self.logger.debug(
                            "Transcript: -> #5b2 (append previous missing)"
                        )
                        tokens.append(self.get_missing(store.pop(0)))
                        tokens.append(variants["".join(store)])
                    else:
                        self.logger.debug(
                            "Transcript: -> #5b3 (append complete missing)"
                        )
            ...
            loopcount += 1
            if loopcount > self._max_iterate_length:  # pragma: no cover
