...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Dumps debug data for a given file'
    )
    parser.add_argument("filename", help="filename")
    parser.add_argument(
        '-q', "--quiet", dest='quiet',
        help="be quiet", action='store_true'
    )
    parser.add_argument(
        '-x', "--explode", dest='explode',
        help="drop into IPython", action='store_true'
    )
    ...
    if not args.quiet or sum(mcount.values()) > 0:
        print("")
        print("MISSING: %s (errors=%d/%d): missing=%r  expected=%r" % (
            args.filename,
            sum(mcount.values()),
            total,
            dict(mcount),
